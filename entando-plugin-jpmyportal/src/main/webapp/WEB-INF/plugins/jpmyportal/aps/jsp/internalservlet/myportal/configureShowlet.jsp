...
<div class="configureShowlet">
	...
	<form action="<wp:action path="/ExtStr2/do/Front/jpmyportal/MyPortal/selectFrame.action" />" method="post" >
		...
		<wpsf:select useTabindexAutoIncrement="true" name="choosenShowlet" id="choosenShowlet" headerKey="unassigned" list="customizableShowlets" listKey="code" listValue="%{getTitle(code, titles)}" />
		<wpsf:submit useTabindexAutoIncrement="true" action="selectFrame" value="%{#labelSubmit}" title="%{#labelSubmit}" cssClass="button" />
		</p>
	</form>
	...
	<s:if test="null != #defaultShowlet">
		<form action="<wp:action path="/ExtStr2/do/Front/jpmyportal/MyPortal/resetFrame.action" />" method="post" >
			...
			<wpsf:submit useTabindexAutoIncrement="true" action="resetFrame" value="%{#labelSubmit}" title="%{#labelSubmit}" cssClass="button" /></p>
		</form>
	</s:if>
</div>