			...
			<wp:action path="/ExtStr2/do/jpcontentfeedback/FrontEnd/contentfeedback/insertVote.action" var="insertVoteAction">
				...
				<form action="<c:out value="${insertVoteAction}" escapeXml="false" />" method="post" class="form-inline">
					...
					<s:iterator value="votes" var="voteItem">
						...
						<wpsf:submit useTabindexAutoIncrement="true" action="insertVote" value="%{#labelSubmit}" cssClass="btn" />
				</form>
			</c:if>
			<%--FINE FORM --%>
			</div>
									...
									<wp:action path="/ExtStr2/do/jpcontentfeedback/FrontEnd/contentfeedback/insertVote.action" var="insertVoteAction" />
										<form action="<c:out value="${insertVoteAction}" escapeXml="false" />" method="post" class="form-inline">
											<p class="noscreen">
												...
												<wpsf:submit useTabindexAutoIncrement="true" action="insertCommentVote" value="%{#labelSubmit}" cssClass="btn" />
										</form>
									</c:if>
								</div>
							</c:if>
								...
								<wp:ifauthorized permission="jpcontentfeedback_comment_edit">
									<p class="comment_actions">
										<a href="<wp:action path="/ExtStr2/do/jpcontentfeedback/FrontEnd/contentfeedback/delete.action">
												...
												</wp:action>" class="btn btn-danger"><i class="icon-remove icon-white"></i>&#32;<wp:i18n key="jpcontentfeedback_DELETE" />
										</a>
									</p>
								</wp:ifauthorized>
							</c:if>
