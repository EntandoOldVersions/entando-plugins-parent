...
<s:if test="#attribute.symbolicLink != null">
	...
	<%-- LINK VALORIZZATO CORRETTAMENTE --%>
		...
		<s:if test="#attribute.symbolicLink.destType == 1">
			...
			<s:set name="linkIntro" ><wp:i18n key="jpfastcontentedit_LINK_TO_URL"/></s:set>
			<s:set name="linkDestination" value="%{#attribute.symbolicLink.urlDest}" />
		</s:if>
		...
		<s:elseif test="#attribute.symbolicLink.destType == 2">
			...
			<s:set name="linkIntro" ><wp:i18n key="jpfastcontentedit_LINK_TO_PAGE"/></s:set>
			<s:set name="linkDestination" value="%{#linkedPage.titles[currentLang.code]}" />
		</s:elseif>
		...
		<s:elseif test="#attribute.symbolicLink.destType == 3">
			...
			<s:set name="linkIntro" ><wp:i18n key="jpfastcontentedit_LINK_TO_CONTENT"/></s:set>
			<s:set name="linkDestination" value="%{#attribute.symbolicLink.contentDest + ' - ' + #linkedContent.descr}" />
		</s:elseif>
		...
		<s:elseif test="#attribute.symbolicLink.destType == 4">
			...
			<s:set name="linkIntro" ><wp:i18n key="jpfastcontentedit_LINK_TO_CONTENT"/></s:set>
			<s:set name="linkContentOnPage" ><wp:i18n key="jpfastcontentedit_LINK_TO_CONTENT_ON_PAGE"/></s:set>
			<s:set name="linkDestination" value="%{#attribute.symbolicLink.contentDest + ' - ' + #linkedContent.descr + ', ' + #linkContentOnPage + ': ' + #linkedPage.titles[currentLang.code]}" />
		</s:elseif>
...
<%-- Lingua di DEFAULT --%>
	...
	<s:set name="configure_label" ><wp:i18n key="jpfastcontentedit_CONFIGURE"/></s:set>
	<s:set name="remove_label" ><wp:i18n key="jpfastcontentedit_REMOVE"/></s:set>
