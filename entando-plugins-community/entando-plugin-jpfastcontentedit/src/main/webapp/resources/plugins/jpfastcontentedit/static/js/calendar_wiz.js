 ...
 * 		- 20090331 added title to day names, added intro text for calendar table
 *		- unlocked data field, user now can write it without using the calendar
 * 		- added title to the calendar toggler 
 * 		- removed auto-open when focusing input field
*/
...
var Calendar = new Class({	
	...
	options: {
		...
		prevTitle: '', //prevTitle: 'Previous Month', 
		nextTitle: '', //nextTitle: 'Next Month',
		prevText: 'Previous', 
		nextText: 'Next',
		introText: "Welcome To Calendar"
	},
	...
	initialize: function(obj, options) {
		...
		this.fx = new Fx.Tween(this.calendar,'opacity', {
			onStart: function() { 
		...
		if (window.Drag && this.options.draggable) {
			this.drag = new Drag.Move(this.calendar, { 
				onDrag: function() {
					if (Browser.Engine.trident4) { this.iframe.setStyles({ left: this.calendar.getStyle('left'), top: this.calendar.getStyle('top') }); } 
				}.bind(this) 
			}); 
		}
		...
		for (var i in obj) {
			var cal = { 
				button: new Element('button', { 'type': 'button', 'title': this.options.calendarTitle }).addEvent("click",function(e){e.preventDefault();}), 
				el: $(i),
				els: [],
				id: id++,
				month: d.getMonth(),
	...
	caption: function(cal) {
		...
		//nextTitle: 'Next'
 		...
 		var prevEl = new Element('span', {'title': this.options.prevTitle, 'class': 'noscreen' }).appendText(this.options.prevText+" ");
 		var nextEl = new Element('span', {'title': this.options.nextTitle, 'class': 'noscreen' }).appendText(" "+this.options.nextText);
 		
		var prev = new Element('a').addClass(this.classes.prev); // <		
		var next = new Element('a').addClass(this.classes.next); // >
	...
	display: function(cal) {
		...
		var introElement = new Element('p', {"class": "noscreen"}).appendText(this.options.introText);;
 		introElement.inject(div); 
		...
		for (var i = 0; i <= 6; i++) {
			...
			var thEl = new Element('th');
			var abbr = new Element ('abbr', { 'title': th}).appendText(th.substr(0, 1));
			thEl.adopt(abbr);
			tr.adopt(thEl);
		}
		...
		//var tr = new Element('tr').inject(tbody);  andrea comment: empty tr... do we need this?...
		...
		cal.els.each(function(el) {			
			...
			*/
			...
			if (el.get('tag') == 'select' && el.retrieve('format').test('^(d|j)$')) { // special case for days-only select
				...
				if (!d) { d = el.get('value').toInt(); } // if the calendar doesn't have a set value, try to use value from select
	...
	toggle: function(cal) {
		...
		if (cal.visible) { 
			...
			// otherwise show (may have to hide others)
			...
			this.fn = function(e, cal) { 
				...
				var stop = false;
 				...
 				if (el != document.html) {  //fix clicking on "small" documents
			...
			if (Browser.Engine.trident4) { this.iframe.setStyles({ left: x + 'px', top: y + 'px' }); }
