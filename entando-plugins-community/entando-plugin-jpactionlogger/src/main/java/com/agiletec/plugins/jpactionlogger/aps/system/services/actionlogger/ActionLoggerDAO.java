...
public class ActionLoggerDAO extends AbstractDAO implements IActionLoggerDAO {
	...
	public void addActionRecord(ActionRecord actionRecord) {
		...
		try {
			...
			processDaoException(t, "Error on insert jpactionlogger record", "addActionRecord");
		} finally {
	...
	public List<Integer> getActionRecords(IActionRecordSearchBean searchBean) {
		...
		try {
			...
			processDaoException(t, "Error loading jpactionlogger records", "getActionRecords");
		} finally {
		...
		try {
			...
			if (null != res) {
				if (res.next()) {
					...
					actionRecord.setParams(res.getString("params"));
			...
			this.executeRollback(conn);
			processDaoException(t, "Error loading jpactionlogger record with id: " + id, "getActionRecord");
		} finally {
			closeDaoResources(res, stat, conn);
		}
		return actionRecord;
	...
	private final String SELECT_ACTION_RECORDS = 
		"SELECT id FROM jpactionlogger_records ";
	...
	private final String APPEND_PARAMS_CLAUSE  = "params LIKE ? ";
	...
	private static final String ADD_ACTION_RECORD = 
		"INSERT INTO jpactionlogger_records ( id, username, actiondate, namespace, actionname, params ) " +
		"VALUES (?, ?, ?, ?, ?, ?)";
	
	private static final String GET_ACTION_RECORD = 
		"SELECT username, actiondate, namespace, actionname, params " +
		"FROM jpactionlogger_records " +
		"WHERE id = ?";
	
	private static final String DELETE_RECORD = 
		"DELETE from jpactionlogger_records where id=?";
