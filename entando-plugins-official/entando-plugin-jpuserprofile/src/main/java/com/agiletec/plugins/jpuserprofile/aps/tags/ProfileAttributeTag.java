...
public class ProfileAttributeTag extends OutSupport {
	...
	public int doStartTag() throws JspException {
		try {
			IUserManager userManager = (IUserManager) ApsWebApplicationUtils.getBean(SystemConstants.USER_MANAGER, this.pageContext);
			UserDetails user = (UserDetails) userManager.getUser(this.getUsername());
			if (user == null || user.getUsername().equals(SystemConstants.GUEST_USER_NAME) || null == user.getProfile()) {
				ApsSystemUtils.getLogger().severe("User '" + user + "' : Null user, or guest user or user without profile");
