var	jpsharewith_buttons = [
    {	'title' : 'Linkedin',
    	'link'  : 'http://www.linkedin.com/shareArticle?mini=true&url=§URL§&title=§TITLE§&summary=§TITLE§&source=',
    	'image' : 'linkedin.png'	
    	},
	{	'title' : 'Digg', 
		'link' : 'http://digg.com/submit?url=§URL§&title=§TITLE§',
	    'image' : 'digg.gif'
	},
	{ 	'title' : 'Furl', 
		'link' : 'http://furl.net/storeIt.jsp?u=§URL§&t=§TITLE§',
		'image' : 'furl.png'
	},
	{ 	'title' : 'StumbleUpon', 
		'link' : 'http://www.stumbleupon.com/submit?url=§URL§&title=§TITLE§',
		'image' : 'su.png'		
	},
	{ 	'title' : 'Technorati', 
		'link' : 'http://www.technorati.com/faves?add=§URL§',
		'image' : 'technorati.gif'		
	},
	{ 	'title' : 'Del.icio.us', 
		'link' : 'http://delicious.com/save?url=§URL§&title=§TITLE§',
		'image' : 'delicious.gif'
	},
	{ 	'title' : 'Facebook', 
		'link' : 'http://www.facebook.com/sharer.php?u=§URL§',
		'image' : 'facebook.gif'		
	},
	{ 	'title' : 'Google', 
		'link' : 'http://www.google.com/bookmarks/mark?op=edit&bkmk=§URL§&title=§TITLE§',
		'image' : 'google.png'		
	},
	{ 	'title' : 'Windows Live', 
		'link' : 'https://favorites.live.com/quickadd.aspx?marklet=1&url=§URL§&title=§TITLE§&top=1',
		'image' : 'windowslive.png'
	},
	{ 	'title' : 'Yahoo', 
		'link' : 'http://myweb2.search.yahoo.com/myresults/bookmarklet?u=§URL§&t=§TITLE§',
		'image' : 'yahoo.png'
	},
	{ 	'title' : 'Slashdot', 
		'link' : 'http://slashdot.org/bookmark.pl?url=§URL§&title=§TITLE§',
		'image' : 'slashdot.gif'		
	},
	{ 	'title' : 'MySpace', 
		'link' : 'http://www.myspace.com/index.cfm?fuseaction=postto&t=§TITLE§&c=&u=§URL§&l=10',
		'image' : 'myspace.png'
	},
	{ 	'title' : 'Virgilio Notizie', 
		'link' : 'http://oknotizie.virgilio.it/post?url=§URL§&title=§TITLE§',
		'image' : 'virgilio.gif'	
	}
];
