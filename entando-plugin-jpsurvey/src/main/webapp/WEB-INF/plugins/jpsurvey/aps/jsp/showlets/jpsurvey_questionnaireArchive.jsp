...
<div class="polls">
<p><wp:i18n key="JPSURVEY_POLLS_ARCHIVE_INTRO" />&#32;<a href="<wp:url page="surveys_list" />" title="<wp:i18n key="JPSURVEY_GO_TO_ACTIVE_QUESTIONNAIRE" />"><wp:i18n key="JPSURVEY_ACTIVE_QUESTIONNAIRE" /></a>.</p>
...
<c:forEach var="currentSurveyItem" items="${pollList}">
	...
	<c:choose>
		<c:when test="${not empty currentSurvey.titles[currentLang]}">
			<a href="<wp:url page="survey_detail"><wp:parameter name="surveyId"><c:out value="${currentSurvey.id}"/></wp:parameter></wp:url>" title="<c:out value="${currentSurvey.titles[currentLang]}"/>"><c:out value="${currentSurvey.titles[currentLang]}"/></a>
		</c:when>
		<c:otherwise>
			<a href="<wp:url page="survey_detail"><wp:parameter name="surveyId"><c:out value="${currentSurvey.id}"/></wp:parameter></wp:url>" title="<c:out value="${currentSurvey.titles[defaultLang]}"/>"><c:out value="${currentSurvey.titles[defaultLang]}"/></a>
		</c:otherwise>
	</c:choose>		
	</h2>
	<p>
